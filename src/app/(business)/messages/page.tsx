...
export default function MessagesPage() {
  return (
    <>
      <PageHead crumb={[{ label: "Home", href: "/" }, { label: "Messages" }]} eyebrow="Leadership" title="Messages" intro="Guidance and vision from the leadership of Garrison Academy Kharian Cantt." />
      <div className="sec"><div className="wrap">
        <div className="grid" style={{ display: "grid", gap: 26 }}>
          {LEADERS.map((l) => (
            <div className="card person" key={l.nm} style={{ textAlign: "left" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
                <div className="ph" style={{ margin: 0, flex: "0 0 auto" }}>{l.ph}</div>
                <div>
                  <div className="nm" style={{ marginTop: 0 }}>{l.nm}</div>
                  <div className="role">{l.role}</div>
                </div>
              </div>
              <div className="msg">
                {l.msg.map((p, i) => (
                  <p key={i} style={{ marginBottom: i === l.msg.length - 1 ? 0 : 12 }}>{p}</p>
                ))}
              </div>
            </div>
          ))}
        </div>
        <div className="note-box mt2">Each message displays the leader&apos;s <b>name, designation and full message</b> — managed from the admin panel and updated whenever leadership changes.</div>
      </div></div>
    </>
  );
}
